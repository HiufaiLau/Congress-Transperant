...
function showTable() { //this is creating the whole table
	...
	var myTable = document.getElementById("Table"); //this new table will connect to html table id 
...
for (i = 0; i < statistic.less_10_pct_missed_votes.length; i++) {
	...
	//		 for (k = 0; k < myMembers.length; k++) {  //
	//        var name = document.createElement("a"); //create the options 
	//        name.setAttribute("href", "myMembers[k].url"); //use the attribute of the option and put the states inside  
	//        name.innerHTML = "myMembers[k].url"; //is a DOM property to insert content to a specified id (#filterTheState)of an element.
	//        myTable.append(name);
	//
	//	}
}
...
function numOfReps(myMem) {
	...
	statistic.total.votes_with_party_pct =
		Math.round((countAverage(republicansArray, statistic.republican.number_of_Reps) +
			countAverage(democratArray, statistic.democrat.number_of_Reps)) / 3 * 100) / 100;
...
function missVote(myMemMissVote) {
	...
	var listOfMissVote = myMemMissVote.sort((a, b) => b.missed_votes_pct - a.missed_votes_pct);
