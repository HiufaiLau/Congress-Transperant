...
function showTable() { //this is creating the whole table
	...
	//table for Least Loyal (Bottom 10% Attendance)
	var myTable = document.getElementById("mostLoyalTable");
	for (i = 0; i < statistic.most_10_pct_party_votes.length; i++) {
		var row = document.createElement("tr");
		var middleName = statistic.most_10_pct_party_votes[i].middle_name;
		if (statistic.most_10_pct_party_votes[i].middle_name == null) { //if the middlename is null , show an empty space 
			middleName = " ";
		} else {
			statistic.most_10_pct_party_votes[i].middle_name; // if there is middlename , then show it on table row
		}
		var link = statistic.most_10_pct_party_votes[i].url;
		var fullName = (statistic.most_10_pct_party_votes[i].first_name + middleName + statistic.most_10_pct_party_votes[i].last_name);
		row.insertCell().innerHTML = "<a href=" + link + " target='_blank'>" + fullName + "</a>";
		row.insertCell().innerHTML = statistic.most_10_pct_party_votes[i].total_votes;
		row.insertCell().innerHTML = statistic.most_10_pct_party_votes[i].votes_with_party_pct+ " "+"%";
		myTable.append(row);
	}

	//table for less loyal (least 10% Attendance)
	var myTable = document.getElementById("lessLoyalTable")
	for (i = 0; i < statistic.less_10_pct_party_votes.length; i++) {
		var row = document.createElement("tr");
		var middleName = statistic.less_10_pct_party_votes[i].middle_name;
		if (statistic.less_10_pct_party_votes[i].middle_name == null) {
			middleName = " ";
		} else {
			statistic.less_10_pct_party_votes[i].middle_name;
		}
		var link = statistic.less_10_pct_party_votes[i].url;
		var fullName = (statistic.less_10_pct_party_votes[i].first_name + middleName + statistic.less_10_pct_party_votes[i].last_name);
		row.insertCell().innerHTML = "<a href=" + link + " target='_blank'>" + fullName + "</a>";
		row.insertCell().innerHTML = statistic.less_10_pct_party_votes[i].total_votes;
		row.insertCell().innerHTML = statistic.less_10_pct_party_votes[i].votes_with_party_pct +" " +"%";
		myTable.append(row);
...
//		countAverage(independentArray, statistic.independent.number_of_Reps)
	...
	statistic.total.votes_with_party_pct =
		Math.round((countAverage(republicansArray, statistic.republican.number_of_Reps) +
			countAverage(democratArray, statistic.democrat.number_of_Reps) ) / 2 * 100) / 100;
...
function loyalVote(memLoyalVote) {
	//least 10 % engage members
	var listOfLoyalVote = memLoyalVote.sort((a, b) => b.votes_with_party_pct - a.votes_with_party_pct);
	console.log(listOfMissVote)

	statistic.most_10_pct_party_votes = memLoyalVote.slice(0, Math.round(memLoyalVote.length * 0.1));
	console.log(statistic.most_10_pct_party_votes)
	//most 10 % engage members
	var listOfMissVote = memLoyalVote.sort((a, b) => a.votes_with_party_pct - b.votes_with_party_pct);
	console.log(listOfMissVote)

	statistic.less_10_pct_party_votes = memLoyalVote.slice(0, Math.round(memLoyalVote.length * 0.1));
	console.log(statistic.less_10_pct_party_votes)
}
