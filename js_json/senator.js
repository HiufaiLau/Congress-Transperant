...
var myMembers = data.results[0].members; //this is my full array
...
function showThisTable(membersInPrint) { //this is creating the whole table
	...
	var myTable = document.getElementById("senatorTable"); //this new table will connect to html table id
...
function changeOption(state) {
	...
	var selectFilter = document.getElementById("filterTheState"); // refer to select tag in html
...
function filterTable(box, opt) {
	...
	if (box.length == 0 && opt == "all") { //if nothing is chosen
		...
		var newTable = myMembers;
	} else if (box.length > 0 && opt == "all") { //only checkboxes are chosen
		console.log('scenario2')
		var newTable = myMembers.filter(oneMember => box.includes(oneMember.party))
	} else if (opt != "all" && box.length == 0) { //only options are chosen
		console.log('scenario3')
		var newTable = myMembers.filter(oneMember => opt == oneMember.state)
...
function changeEventHandler() {
	...
	var myOption = document.getElementById("filterTheState"); // the option "string" id refer to html
